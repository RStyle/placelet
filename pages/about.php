        <article id="about-us" class="mainarticles bottom_border_blue">
			<div class="blue_line mainarticleheaders line_header"><h1>Über uns</h1></div>
			<img src="pictures/staff.jpg" alt="Mitarbeiter von Placelet" id="wir">
			<h1>Global Bracelet. Travel&Connect</h1>
			<p>Das ist das Ziel unserer Firma <strong>Placelet</strong>, die aus 20 engagierten Teilnehmern des Juniorprojektes 2013 des Deutschen Institut für Wirtschaft Köln (DIW Köln) besteht und ihren Sitz in Bad Kreuznach hat.
			Wir wollen mithilfe unserer Reisearmbändern eine Community gründen, indem Sie von Euch an reisende Freunde und Verwandte oder auch zufällig getroffene Menschen weitergeben werden.</p>
		</article>
		<aside class="side_container">
			<h1>JUNIOR</h1>
			<p>JUNIOR ist ein Projekt der Institut der deutschen Wirtschaft Köln JUNIOR gGmbH. JUNIOR wird 
			auf Bundesebene durch das Bundesministerium für Wirtschaft und Technologie, die KfW Mittelstandsbank, Gesamtmetall, dem Handelsblatt, Danfoss, Deloitte, der AXA Versicherung und Fed 
			Ex gefördert.</p>
		</aside>