<?php
function commentPush($commenter, $brid, $picid, $comment, $token) {
	$data = array(
		'type' => 'comment',
		'commenter' => $commenter,
		'brid' => $brid,
	...
	return sendGoogleCloudMessage($data, $token);
}
function picPush($uploader, $braceName, $brid, $picid, $token) {
	$data = array(
		'type' => 'pic',
		'uploader' => $uploader,
		'braceName' => $braceName,
...
function messagePush($sender, $senderID, $content, $token) {
	$data = array(
		'type' => 'message',
		'sender' => $sender,
		'content' => $content,
...
function sendGoogleCloudMessage($data, $ids)
{
	...
	$apiKey = 'AIzaSyA4-mokQ3QMCBw5s4E8M4ZIkhxB3ZpKOyg';
	...
	$result = curl_exec($ch);
	if(curl_errno($ch)) return false;//echo 'GCM error: ' . curl_error( $ch );
	curl_close($ch);
	
	//echo $result;
	$jsonResult = json_decode($result, true);
	if(isset($jsonResult['results'][0]['registration_id'])) {
		$sql = "UPDATE users SET androidToken = :newToken WHERE androidToken = :oldToken";
		$q = $this->db->prepare($sql);
		$q->execute(array(
			':newToken' => $notificPushed,
			':oldToken' => $result['androidToken'],
		));
		return $jsonResult['results'][0]['registration_id'];
	}elseif($jsonResult['success'] == 1) {
		return true;
	}else return $result;
}
...
function messagePushBots($sender, $token) {
		$pb = new PushBots();
		$appID = '539986981d0ab1d0048b45f6';
		$appSecret = 'eeb459d33ad5c6bd81474a7c7484cc06';
		$pb->App($appID, $appSecret);
